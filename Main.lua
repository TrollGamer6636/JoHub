...
function showCatalogContent(index)
    ...
    if index == 1 then
        ...
        -- Begrüßung unter dem Avatar (zentriert im Feld)
        ...
        welcome.Position = UDim2.new(0.5, 0, 0.5, 24) -- mittig, etwas unter dem Avatar
        welcome.Size = UDim2.new(1, -10, 0, 24)
        ...
        feedbackBtn.MouseButton1Click:Connect(function()
            ...
            sendBtn.MouseButton1Click:Connect(function()
                ...
                local sent = false
                local errorMsg = nil
                -- Exploit HTTP Methoden
                local function showThanks(success, err)
                    if success then
                        fbBox.Visible = false
                        ...
                        sendBtn.Active = true
                        sendBtn.Text = "Senden"
                        showNotify("Fehler: "..(err or "Unbekannt"), Color3.fromRGB(255,60,60), modal, 2)
                    end
                end
                    ...
                    local ok, err = pcall(function()
                        if syn and syn.request then
                            ...
                            if res and res.StatusCode and res.StatusCode >= 200 and res.StatusCode < 300 then
                                ...
                                errorMsg = "syn.request HTTP "..tostring(res and res.StatusCode or "?")
                            end
                        elseif http_request then
                            ...
                            if res and res.StatusCode and res.StatusCode >= 200 and res.StatusCode < 300 then
                                ...
                                errorMsg = "http_request HTTP "..tostring(res and res.StatusCode or "?")
                            end
                        elseif request then
                            local res = request({
                                Url = DISCORD_WEBHOOK,
                            ...
                            if res and res.StatusCode and res.StatusCode >= 200 and res.StatusCode < 300 then
                                ...
                                errorMsg = "request HTTP "..tostring(res and res.StatusCode or "?")
                            end
                        else
                            -- Roblox HttpService (funktioniert meist NICHT für Discord)
                            ...
                            if ok2 then
                                ...
                                errorMsg = "Roblox HttpService: "..tostring(err2)
                            end
                        end
                    end)
                    if not sent then
                        showThanks(false, errorMsg or err)
                    end
                end
                trySend()
            end)
